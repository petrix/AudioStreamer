...
(function (exports) {
	/*! binarypack.js build:0.0.9, production. Copyright(c) 2012 Eric Zhang <eric@ericzhang.com> MIT Licensed */ (function e(t, n, r) {
		function s(o, u) {
			if (!n[o]) {
				if (!t[o]) {
			...
			1: [
				function (require, module, exports) {
					...
					Unpacker.prototype.unpack_uint64 = function () {
						...
						var uint64 =
							((((((bytes[0] * 256 + bytes[1]) * 256 + bytes[2]) * 256 + bytes[3]) * 256 + bytes[4]) * 256 + bytes[5]) * 256 + bytes[6]) * 256 + bytes[7];
						this.index += 8;
						return uint64;
					};
	...
	function BinaryClient(socket, options) {
		...
		this._socket.addEventListener("message", function (data, flags) {
			util.setZeroTimeout(function () {
				...
				if (!(data instanceof Array)) return self.emit("error", new Error("Received non-array message"));
				if (data.length != 3) return self.emit("error", new Error("Received message with wrong part count: " + data.length));
				if ("number" != typeof data[0]) return self.emit("error", new Error("Received message with non-number type: " + data[0]));
